...
class MainWindow(QMainWindow):
    ...
    def addScript(self, filename):
        ...
        if filename not in self.scripts:
            ...
            self.importScript(filename)
    
    def importScript(self, filename):
        """Imports the script into the global namespace."""
        script_name = os.path.basename(filename)[:-3]
        try:
            if filename not in self.scripts:  # Check if the script is not already added
                # Convert PySide2 script to PyQt5 dynamically
                ...
                exec(script_code, self.global_namespace)
                
                # Add the imported objects to the tab widget
                if 'get_tab_widget' in self.global_namespace:
                    tab_content = self.global_namespace['get_tab_widget']()
                else:
                    tab_content = QWidget()
    ...
    def removeScript(self, filename):
        if filename in self.scripts:
            ...
            # Remove imported objects from global namespace
            for key in list(self.global_namespace.keys()):
                if key.startswith('__') and key.endswith('__'):
                    continue  # Skip built-in objects
                if self.global_namespace[key].__module__ == filename[:-3]:
